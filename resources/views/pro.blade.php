...
<body>
    ...
    <div class="container mt-5 mb-5">
        <div class="card">
            <div class="row g-0">
                <div class="col-md-12 border-end">
                    <div class="d-flex flex-column justify-content-center">
                        <div class="main_image"> <img src="{{ asset('imgs/' . $pro->img) }}" id="main_product_image"
                                width="350"> </div>
                        <div class="thumbnail_images">
                            <div class="ratings d-flex flex-row align-items-center"
                                style="display: flex; align-items: center; text-align: center; justify-content: center;">
                                {{ trans_choice('{0} no like|{1} :count like|[2,*] :count likes', count($pro->likes), ['count' => count($pro->likes)]) }}

                                @can('like', $pro)
                                    <form action="{{ route('like') }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="likeable_type" value="{{ get_class($pro) }}" />
                                        <input type="hidden" name="id" value="{{ $pro->id }}" />
                                        <button><img src="{{ asset('image/heart.png') }}" alt=""
                                ...
                                @can('unlike', $pro)
                                    <form action="{{ route('unlike') }}" method="POST">
                                        ...
                                        <input type="hidden" name="likeable_type" value="{{ get_class($pro) }}" />
                                        <input type="hidden" name="id" value="{{ $pro->id }}" />
                                        <button><img src="{{ asset('image/heart1.png') }}" alt=""
                                                style="width: 20px; height:20px"></button>
                                    </form>
                                @endcan
                            </div>
                    ...
                    <div class="col-md-10">
                        <div class="p-3 right-side" style="width:100%">
                            <div class="d-flex justify-content-between align-items-center">
                                <h3>{{ $pro->name }}</h3>
                                <h3 style="color: #0d6efd; margin-right:40px">{{ $pro->price }} DH</h3>
    
                            </div>
                            <div style="display: flex">
                                <img src="../image/loc.webp" alt="" style="width: 25px; height:25px; margin-right:5px">
                            <h5 style="margin-right: 10px">{{$pro->origin}}</h5>
                            <img src="../image/time.png" alt="" style="width: 25px; height:25px; margin-right:5px">
                            <h5>{{ $pro->created_at->diffForHumans() }}</h5>
                            </div>
                            
                            <div style="display: flex; justify-content: space-between; margin-top:20px; width:100%;">
                            <form action="{{ route('botic') }}" method="GET" style="border: 1px solid black; display: inline-flex;height: 55px;">
                                <img src="../image/Profile.png" alt="" style="width:50px; height:50px">
                                <input type="hidden" name="id" value="{{ $pro->user_id }}">
                                <input  type="submit" style="background: none; width:100px; border :none;" value="{{ \App\Models\User::find($pro->user_id)->name }}">
                            </form>


                            <div class="buttons d-flex flex-row mt-5 gap-3 btn btn-outline-dark" style="top: -50px;position: relative; background-color:#0d6efd; border-radius:30px; width:200px; height:60px"> 
                                <a href="{{ route('user', $pro->user_id) }}">
                                    <img src="../image/contact.png" alt="" style="width:50px; height:50px">
                            ...
                            <div class="mt-2 pr-3 content" style="margin-left: 40px">
                                <p style="font-size: 18px">{{ $pro->desc }}</p>
                            </div>
                ...
                @auth
                <section style="background-color: #dad5d4;">
                              ...
                              <div class="d-flex flex-start w-100">
                                ...
                                <div class="w-100">
                                  ...
                                  <ul class="rating mb-3" data-mdb-toggle="rating">
                                    ...
                                    <form method="POST" action="{{route('comments.store', $pro)}}">
                                        @csrf
                                  <div class="form-outline">
                                    <textarea class="form-control" name="comm" rows="4"></textarea>
                                    <input type="hidden" name="usri" value="{{Auth::user()->id }}">
                                    <input type="hidden" name="usrn" value="{{Auth::user()->name }}">
            ...
            <h3 style="text-align: center; color:rgb(162, 162, 226); margin:20px;">Similar product</h3>
                                    ...
                                    <div class="text-center">
                                        <form action="{{ route('product.show', $pro->id) }}" method="GET">
                                            ...
                                            <input type="hidden" name="vendeur"
                                                value="{{ \App\Models\User::find($pro->user_id)->name }}">
                ...
                {{-- comments section --}}
                <h3 style="text-align: center; color:rgb(162, 162, 226); margin:20px;">Comments</h3>
                @foreach ($comments as $comm)

                    <div class="row d-flex justify-content-center m-10" style="margin: 10px;">
                            ...
                            <div class="card-body p-4">
                                ...
                                <div class="d-flex justify-content-between">
                                    <div class="d-flex flex-row align-items-center">
                                      <p class="small mb-0 ms-2"> <b>{{$comm->name}}</b> </p>
                                    </div>
                                  </div>
                ...
                @endforeach
                <footer style="background-color: rgb(13, 55, 94); height:25px; width:100%">
